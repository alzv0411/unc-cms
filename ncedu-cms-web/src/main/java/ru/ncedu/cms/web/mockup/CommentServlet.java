 ...
 */
@WebServlet(name = "CommentServlet", urlPatterns = {"/mockup/comment"})
public class CommentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isNotEmpty(commentId)) {
            ...
            if (comment == null) {
                request.setAttribute("message", "Comment with id=" + commentId + " doesn't exist");
                request.getRequestDispatcher("error.jsp").forward(request, response);
            }
            
            request.setAttribute("comment", comment);
            request.getRequestDispatcher("/views/mockup/comment.jsp").forward(request, response);
        }

        List<Comment> comments = commentDAO.findAll();
        request.setAttribute("comments", comments);
        request.getRequestDispatcher("/views/mockup/comments.jsp").forward(request, response);
    }
